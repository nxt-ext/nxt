...
QUnit.test("select", function (assert) {
    init();
    NRS.storageSelect("items", "id", null, function(error, response) {
    ...
    NRS.storageSelect("items", "id", [{ id:"1"}, {id:"3"}], function(error, response) {
        assert.equal(error, null);
...
QUnit.test("select.non.existent", function (assert) {
    init();
    NRS.storageSelect("dummy", "id", null, function(error, response) {
...
QUnit.test("update", function (assert) {
    init();
    NRS.storageUpdate("items", "id", [{"id": "3", name: "qqq"}], [{id:"3"}], function() {
        NRS.storageSelect("items", "id", null, function(error, response) {
            assert.equal(error, null);
            assert.equal(response.length, 3);
...
QUnit.test("delete", function (assert) {
    init();
    NRS.storageDelete("items", "id", [{"id": "2"}], function() {
        NRS.storageSelect("items", "id", null, function(error, response) {
            assert.equal(error, null);
            assert.equal(response.length, 2);
            assert.equal(response[0]["id"], "1");
    ...
    NRS.storageDelete("items", "id", [{"id": "4"}], function() {
        NRS.storageSelect("items", "id", null, function(error, response) {
            assert.equal(error, null);
            assert.equal(response.length, 2);
        })
    });
    NRS.storageDelete("items", "id", [{"id": "3"}], function() {
        NRS.storageSelect("items", "id", null, function(error, response) {
            assert.equal(error, null);
