...
public class MintWorker {
    ...
    private void mint() {
        ...
        while (true) {
            counter++;
            JSONObject response = mintImpl(currencyCode, secretPhrase, accountId, units, currencyId, algorithm, counter, target, initialNonce, threadPoolSize, executorService, difficulty);
    ...
    private JSONObject mintImpl(String currencyCode, String secretPhrase, long accountId, long units, long currencyId, byte algorithm, long counter, byte[] target, long initialNonce, int threadPoolSize, ExecutorService executorService, long difficulty) {
